...
import "mocha";
import { LampsModel } from "../../src/models/LampsModel";
...
describe("LampsModel", () => {
    ...
    before(async () => {
        ...
        model = new LampsModel();
        lampId = "ICE-003-042-000-002";
    ...
    it("should return fulfilled promise to GetOne call", async () => {
        const promise = model.GetOne(lampId);
        expect(Object.prototype.toString.call(promise)).to.equal("[object Promise]");
        await expect(promise).to.be.fulfilled;
    });

    it("should return one lamp by id", async () => {
        const data = await model.GetOne(lampId);
        expect(data).to.be.an.instanceOf(Object);
    });

    it("should throw an error (reject promise) for non-existing lamp by id", async () => {
    ...
    it("should return at least 2 records, sorted by the closest one (GetByCoordinates)", async () => {
        ...
        // TODO: Check for real distance difference, this is bullshit (at least one coordinate has to be greater in the second record)
